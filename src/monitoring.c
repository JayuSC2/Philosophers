...
/*   Updated: 2024/09/02 16:14:40 by juitz            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int monitoring(t_metadata *m_data)
{
    ...
    while (1)
    {
        ...
        {
			//pthread_mutex_lock(&m_data->time->time_lock);
            m_data->time->time_passed = get_actual_time(m_data->time);
			//pthread_mutex_unlock(&m_data->time->time_lock);
            ...
            {
                return (print_status(&m_data->philo[i], "died"),m_data->death_flag = true, pthread_mutex_unlock(&m_data->death_lock), 1);
            }
